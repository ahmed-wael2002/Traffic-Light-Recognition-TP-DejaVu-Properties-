 ...
 */
 ...
 implicit class ExtendedFloat(val a: Float) extends AnyVal {
  ...
  object Operators {
	...
	private var vAccel: Int = 0
	private var accelLimit: Int = 0


	private def on_AccelerationChanged(vAccel: Int, accelLimit: Int): Unit = {
	}

	private def AccelerationChanged_output(): Any = {
		List("checkAccelerationBelowLimit", List(vAccel.toString, accelLimit.toString))
	}
	...
	def evaluate(event_name: String, params: Any*): Option[Any] = {
		...
		event_name match {
			case "AccelerationChanged" => {
				if (params.length != 2) throw new IllegalArgumentException("Incorrect number of parameters for event AccelerationChanged")
				Try(params(0).toString.trim.toInt) match {
					case Success(value) => this.vAccel = value
					case Failure(e) => println(s"Failed to convert to Int: $e")
				}
				Try(params(1).toString.trim.toInt) match {
					case Success(value) => this.accelLimit = value
					case Failure(e) => println(s"Failed to convert to Int: $e")
				}
				on_AccelerationChanged(vAccel, accelLimit)
				event = AccelerationChanged_output()
			}
...
/*
  prop VehicleAccelCheck : forall vAccel . forall accelLimit . checkAccelerationBelowLimit(vAccel,accelLimit) -> vAccel <= accelLimit 
*/

class Formula_VehicleAccelCheck(monitor: Monitor) extends Formula(monitor) {
          
  override def evaluate(): Boolean = {
    // assignments1 (leaf nodes that are not rule calls):
      now(3) = build("checkAccelerationBelowLimit")(V("vAccel"),V("accelLimit"))
    ...
    // assignments5 (main formula excluding leaf nodes):
      now(4) = relation("vAccel",LEOP,"accelLimit").or(pre(4))
      now(2) = now(3).not().or(now(4))
      now(1) = var_accelLimit.seen.imp(now(2)).forAll(var_accelLimit.quantvar)
      now(0) = var_vAccel.seen.imp(now(1)).forAll(var_vAccel.quantvar)
  ...
  val var_vAccel :: var_accelLimit :: Nil = declareVariables(("vAccel",true), ("accelLimit",true))(0)

  varsInRelations = Set("vAccel","accelLimit")
  ...
  txt = Array(
    "forall vAccel . forall accelLimit . checkAccelerationBelowLimit(vAccel,accelLimit) -> vAccel <= accelLimit",
      "forall accelLimit . checkAccelerationBelowLimit(vAccel,accelLimit) -> vAccel <= accelLimit",
      "checkAccelerationBelowLimit(vAccel,accelLimit) -> vAccel <= accelLimit",
      "checkAccelerationBelowLimit(vAccel,accelLimit)",
      "vAccel <= accelLimit"
  )
...
class PropertyMonitor(preMonitor: PreMonitorTrait) extends Monitor(preMonitor) {
  ...
  def eventsInSpec: Set[String] = Set("checkAccelerationBelowLimit")

  formulae ++= List(new Formula_VehicleAccelCheck(this))
}
