package recognition_accuracy_check
...
class Formula_StateRecognitionCheck(monitor: Monitor) extends Formula(monitor) {
          
  override def evaluate(): Boolean = {
    // assignments1 (leaf nodes that are not rule calls):
      now(3) = build("checkStateRecognition")(V("recState"),V("actState"))
    ...
    // assignments5 (main formula excluding leaf nodes):
      now(4) = relation("recState",EQOP,"actState").or(pre(4))
      now(2) = now(3).not().or(now(4))
      now(1) = var_actState.seen.imp(now(2)).forAll(var_actState.quantvar)
      now(0) = var_recState.seen.imp(now(1)).forAll(var_recState.quantvar)
  ...
  val var_recState :: var_actState :: Nil = declareVariables(("recState",true), ("actState",true))(0)

  varsInRelations = Set("recState","actState")
  ...
  txt = Array(
    "forall recState . forall actState . checkStateRecognition(recState,actState) -> recState = actState",
      "forall actState . checkStateRecognition(recState,actState) -> recState = actState",
      "checkStateRecognition(recState,actState) -> recState = actState",
      "checkStateRecognition(recState,actState)",
      "recState = actState"
  )
