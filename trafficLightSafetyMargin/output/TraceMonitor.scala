...
class BDDGenerator(F: Formula)(variables: List[(String, Boolean, Int)], bitsPerTimeVar: Int) {
  var B: BDDFactory = null
  
  try {
    B = BDDFactory.init(10000, 10000)
  } catch {
    case e: Exception => 
      println("Error initializing BDD factory: " + e.getMessage)
      println("Make sure libbuddy.so is installed and in the library path.")
      println("You may need to install the JavaBDD package or set LD_LIBRARY_PATH.")
      throw e
  }
...
abstract class Monitor(preMonitor: PreMonitorTrait){
  ...
  def submit(name: String, args: List[Any]): Boolean = {
    try {
      ...
      case e: Exception => 
        println(s"Error in submit: ${e.getMessage}")
        e.printStackTrace()
        errors += 1
        false
    }
  ...
  def evaluate(): Boolean = {
    try {
      ...
      case e: Exception => 
        println(s"Error in evaluate: ${e.getMessage}")
        e.printStackTrace()
        errors += 1
        false
    }
...
abstract class Formula(val monitor: Monitor) {
  ...
  case object LTOP extends RelOp {
    def compare(v1: Any, v2: Any): Boolean = {
      v1.asInstanceOf[String].toDouble < v2.asInstanceOf[String].toDouble
    }
 ...
 */
 ...
 implicit class ExtendedFloat(val a: Float) extends AnyVal {
  ...
  object Operators {
	...
	private var DISTANCE_LIMIT: Double = 4
	private var prev_DISTANCE_LIMIT: Double = 4
	private var xSq: Double = 0
	private var prev_xSq: Double = 0
	private var ySq: Double = 0
	private var prev_ySq: Double = 0
	private var distance_limit: Double = 0
	private var prev_distance_limit: Double = 0
	...
	private def on_Red(vX: Double, vY: Double, sX: Double, sY: Double): Unit = {
		this.xSq = (sX - vX) * (sX - vX)
		this.ySq = (sY - vY) * (sY - vY)
		this.distance = xSq + ySq
		this.distance_limit = DISTANCE_LIMIT * DISTANCE_LIMIT
	}

	private def Red_output(): Any = {
		List("checkStopMargin", List(distance.toString, distance_limit.toString))
	}
	...
	def evaluate(event_name: String, params: Any*): Option[Any] = {
		...
		event_name match {
			case "Red" => {
				if (params.length != 4) throw new IllegalArgumentException("Incorrect number of parameters for event Red")
				Try(params(0).toString.trim.toDouble) match {
					case Success(value) => this.vX = value
					case Failure(e) => println(s"Failed to convert to Double: $e")
				}
				Try(params(1).toString.trim.toDouble) match {
					case Success(value) => this.vY = value
					case Failure(e) => println(s"Failed to convert to Double: $e")
				}
				Try(params(2).toString.trim.toDouble) match {
					case Success(value) => this.sX = value
					case Failure(e) => println(s"Failed to convert to Double: $e")
				}
				Try(params(3).toString.trim.toDouble) match {
					case Success(value) => this.sY = value
					case Failure(e) => println(s"Failed to convert to Double: $e")
				}
				on_Red(vX, vY, sX, sY)
				event = Red_output()
		...
		prev_DISTANCE_LIMIT = DISTANCE_LIMIT
		prev_xSq = xSq
		prev_ySq = ySq
		prev_distance_limit = distance_limit
		Some(event)
...
/*
  prop StopMarginCheck : forall distance . forall distance_limit . checkStopMargin(distance,distance_limit) -> distance <= distance_limit 
*/

class Formula_StopMarginCheck(monitor: Monitor) extends Formula(monitor) {
          
  override def evaluate(): Boolean = {
    try {
      // assignments1 (leaf nodes that are not rule calls):
        now(3) = build("checkStopMargin")(V("distance"),V("distance_limit"))
      ...
      // assignments5 (main formula excluding leaf nodes):
        now(4) = relation("distance",LEOP,"distance_limit").or(pre(4))
        now(2) = now(3).not().or(now(4))
        now(1) = var_distance_limit.seen.imp(now(2)).forAll(var_distance_limit.quantvar)
        now(0) = var_distance.seen.imp(now(1)).forAll(var_distance.quantvar)
      ...
      case e: Exception => 
        println(s"Error in Formula_StopMarginCheck.evaluate: ${e.getMessage}")
        e.printStackTrace()
        false
    }
  }

  val var_distance :: var_distance_limit :: Nil = declareVariables(("distance",true), ("distance_limit",true))(0)

  varsInRelations = Set("distance","distance_limit")
  ...
  txt = Array(
    "forall distance . forall distance_limit . checkStopMargin(distance,distance_limit) -> distance <= distance_limit",
      "forall distance_limit . checkStopMargin(distance,distance_limit) -> distance <= distance_limit",
      "checkStopMargin(distance,distance_limit) -> distance <= distance_limit",
      "checkStopMargin(distance,distance_limit)",
...
class PropertyMonitor(preMonitor: PreMonitorTrait) extends Monitor(preMonitor) {
  ...
  def eventsInSpec: Set[String] = Set("checkStopMargin")

  formulae ++= List(new Formula_StopMarginCheck(this))
}
...
object TraceMonitor {
  ...
  def eval(event: String): Boolean = {
    try {
      ...
      case e: Exception => 
        println(s"Error in eval: ${e.getMessage}")
        e.printStackTrace()
        closeResultFile()
        return false
    }
